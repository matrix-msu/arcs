...
if( typeof kids_to_get != 'undefined' && kids_to_get !== "keywordSearch" ){
    //console.log('kids to get', kids_to_get)
	function searchLoading(e){
...
function setUpSearchLoad(keywordSearch = false){
    ...
    var temptop = 20;
    if( temptop > kids_to_get.length ){
        temptop = kids_to_get.length;
    }
    $('#result-info').html('Showing <div id="top-number-per-page" style="display:inline">'+temptop+'</div> of '+kids_to_get.length+' result(s) for ');
    $('#results-count').html(kids_to_get.length);
    $('.sorter.active').removeClass('active');
    $('.dir-btn.active').removeClass('active');
    $('.SearchBar').find('.btn-group').css('opacity','.2').find('button').css('cursor','wait');
    $('#select-all').css('opacity','.2').css('cursor','wait');
...
(function() {
  ...
  arcs.views.search.Search = (function(superClass) {
		...
		$.ajax({
			...
			'success': function(data) {
				...
				for( var resourceKid in data ){
					...
					$(totalResults).each(function(index,value){
						if( value.kid == resourceKid ){
							totalResults[index]['thumb'] = data[resourceKid]['thumb'];
							unfilteredResults[index]['thumb'] = data[resourceKid]['thumb'];
						}
					});
				}
			}
	...
	function getRestOfData(index, limit){
		...
		if( index > kids_to_get.length -1 && kids_to_get.length > 0){
			//finished all loading
			filters.excavations.sort();
			filters.projects.sort();
			filters.seasons.sort();
			filters.sites.sort();
			filters.types.sort();
			setFilters();
			addFilterEvent();
		  ...
		  $.ajax({
			...
			'success': function(data) {
				...
				lastPage = Math.ceil(totalResults.length / numberPerPage);
	...
	if(typeof results_to_display != "undefined" && results_to_display != false){
    ...
    Search.prototype.scrollTop = function() {
      var time;
      time = ($(window).scrollTop() / $(document).height()) * 800;
    ...
    adjustPage = function(currentPage) {
      ...
      skip = (pageNum - 1) * numberPerPage;
      $('#lastPage').html(lastPage);
	  if( typeof kids_to_get != 'undefined'){
		  var pagesToGet = {};
		  $(totalResults.slice(skip, skip + numberPerPage)).each(function(index, value){
				pagesToGet[value.kid] = value;
		  });
		  getRestOfImages(pagesToGet, false);
	  }
    ...
    Search.prototype._render = function(results, append) {
      ...
      $(".pageNumber").unbind().click(function(e) {
        if ($(this).hasClass('selected')) {
          ...
          adjustPage(parseInt($('.currentPage').html()));
      ...
      $('.perpage-btn').unbind().click(function() {
		...
		var temptop = parseInt($(this).html().substring(0, 2));
		if( temptop > kids_to_get.length ){
			temptop = kids_to_get.length;
		}
		$('#top-number-per-page').html(temptop);
        $('.pageNumber').removeClass('selected');
        $('.pageNumber').removeClass('currentPage');
        $("#1").addClass('selected');
        $("#1").addClass('currentPage');
      ...
      filterResults = function() {
		...
		if( totalResults.length < parseInt($('#top-number-per-page').html()) ){
			$('#top-number-per-page').html(totalResults.length);
		}
        adjustPage(1);
      };
		  ...
		  $('.filter').unbind().on("click", function() {
			...
			if ($(this).hasClass('active')) {
			  ...
			  if (filterCnt) {
				...
				$('#top-number-per-page').html(totalResults.length);
				var temptop = parseInt($('#items-per-page-btn').html().substring(0, 2));
				if( temptop > totalResults.length ){
					temptop = totalResults.length;
				}
				$('#top-number-per-page').html(temptop);
				adjustPage(1);
			  }
      ...
      $('.dir-btn').unbind().click(function() {
        ...
        if ($(this).hasClass('active')) {
		  ...
		  if( $('.sorter.active').length == 0 ){
				$('#sort-title-btn').addClass('active');
			}
          return adjustPage(1);
        }
      });
    ...
    $('.sorter').unbind().click(function() {
      ...
      if ($(this).hasClass('active')) {
		...
		if( $('.dir-btn.active').length == 0 ){
				$('#dir-asc-btn').addClass('active');
			}
        return adjustPage(1);
      }
    });
