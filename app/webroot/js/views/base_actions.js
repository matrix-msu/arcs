...
(function() {
  ...
  arcs.views.BaseActions = (function(superClass) {
    extend(BaseActions, superClass);

    function BaseActions() {
      return BaseActions.__super__.constructor.apply(this, arguments);
    }

    BaseActions.prototype.openResource = function(resource) {
      if (this._modelFromRef != null) {
        resource = this._modelFromRef(resource);
      }
    ...
    BaseActions.prototype.editResource = function(resource, attributes) {
      var _metadata, _resource, key, metadata, value;
      metadata = resource.get('metadata');
      _resource = _.clone(resource.attributes);
      _metadata = _.clone(metadata.attributes);
      for (key in attributes) {
        value = attributes[key];
        if (indexOf.call(_.keys(resource.attributes), key) >= 0) {
          resource.set(key, value);
        } else if (metadata.get(key) || value) {
          metadata.set(key, value);
        }
      }
      if (!_.isEqual(resource.attributes, _resource)) {
        resource.save();
      }
