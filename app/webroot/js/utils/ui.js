...
$(document).ready(function() {
    $("#registerModal .selectDiv1").click(function(e) {
        e.stopPropagation();
    ...
    $("#registerModal .selectDiv2").click(function(e) {
        e.stopPropagation();
        $("#projectDropdown2").toggle();
        var value = "";
        $("#projectDropdown2 .selected").each(function(i, e) {
            if (value === "") {
                value = $(e).attr("id");
            }
            else {
        ...
        $("#UserProject2").val(value);
    });

    $("#projectDropdown2 > p").click(function(e) {
        $("#projectDropdown2").toggle();
        e.stopPropagation();
        var target = $(e.target);
        var selectDiv = $(".selectDiv2");
        target.toggleClass('selected');
        if (target.hasClass('selected')) {
            if (selectDiv.text().includes("Select Project(s) to Register In *")) {
                selectDiv.text(target.text());
        ...
        if ($("#projectDropdown2 .selected").length === 0) {
            selectDiv.text("Select Project(s) to Register In *");
        }
    });
