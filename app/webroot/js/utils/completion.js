...
(function() {
  ...
  memoize = function(f, expire) {
    ...
    return function() {
      ...
      key = args.join('');
      time = Date.now();
      if (key in cache && (time - cache[key][0] < expire)) {
        return cache[key][1];
      }
  ...
  arcs.completeDate = function(url) {
    var aliases, d, dates, fmt, parseFmt, raw, ref;
    raw = arcs.complete(url);
    ref = ['DD-MM-YYYY', 'YYYY-MM-DD HH:mm:ss'], fmt = ref[0], parseFmt = ref[1];
    dates = (function() {
      var i, len, results;
      results = [];
      for (i = 0, len = raw.length; i < len; i++) {
        d = raw[i];
        results.push(moment(d, parseFmt).format(fmt));
      }
      return results;
    })();
    aliases = [
  ...
  arcs.utils.autocomplete = function(opts) {
    var $el, addTerm, defaults, focus, getLast, options, ref, select, split;
    defaults = {
      source: [],
      multiple: false,
      sel: null
    ...
    if (_.isFunction(options.source) && options.source.length === 0) {
      ref = [options.source(), options.source], options.source = ref[0], options._source = ref[1];
    }
    if (options.multiple) {
