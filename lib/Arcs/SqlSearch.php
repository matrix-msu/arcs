...
class SqlSearch extends Search {
    ...
    public function __construct($config) {
        ...
        $this->connection = new \PDO("$driver:host=$host;dbname=$db", $login, $pass);
    }
    ...
    private function addAllFacet($value) {
        ...
        $this->addFacet('title', $value, 'or');
        $this->addFacet('type', $value, 'or');
        $this->addFacet('keyword', $value, 'or');
        /*
        foreach ($this->mappings as $facet => $map) {
            if ($facet == 'access') continue;
            if ($facet == 'collection' || $facet == 'collection_id') continue;
